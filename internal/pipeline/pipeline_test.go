...
func TestGroupWithFailure(t *testing.T) {
	cfg := &config.PipelineConfig{
		Name: "test-group-fail",
		Loop: config.LoopConfig{Count: 1},
		Steps: []config.StepConfig{
			{
				Group: "will-fail",
				Loop:  config.LoopConfig{Count: 2},
				Steps: []config.StepConfig{
	...
	p := New(cfg, testLogger)
	if err := p.Run(context.Background()); err == nil {
		t.Error("expected error from failing agent in group")
	}
}
...
func TestLoopCount(t *testing.T) {
	cfg := &config.PipelineConfig{
		Name: "test-loop",
		Loop: config.LoopConfig{Count: 3},
		Steps: []config.StepConfig{
			{Name: "echo", Command: "echo", Args: []string{"loop"}, OnError: "stop"},
		},
	}
